...
The representation that I will use will be a Map<String, String> accounts. A map will allow for easy look ups, additions and removals. accounts cannot be null, all usernames (keys) should be unique and not null. All passwords(values) should not be null.

DOMAIN(this) will be represented as a set of keys.

and this[username] will be represented by accounts.value(username)
